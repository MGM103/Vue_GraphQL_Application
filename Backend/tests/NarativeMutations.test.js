...
describe("Test Narative Queries", () => {
  ...
  test("Test create Narative", async () => {
    ...
    const mutationVars = {
      narative: {
        name: "Ve(3,3)",
        description:
          "Andre Cronje's invention for a sustainable incentive system to promote the growth of a DEX. Borrows from Curve (Ve) and OlympusDAO (3,3)",
      },
    };
    const mutation = gql`
    ...
    expect(result.data.createNarative.description).toEqual(
      mutationVars.narative.description
    );
  });
  ...
  test("Test edit Narative", async () => {
    ...
    expect(result.data.editNarative.description).toEqual(
      mutationVars.narative.description
    );
